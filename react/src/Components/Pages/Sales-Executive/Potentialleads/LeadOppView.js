...
const LeadOppView = ({ show, handleClose, lead }) => {
  return (
    <Modal show={show} onHide={handleClose} size="lg">
      <Modal.Header closeButton className='s-LeadOppView-modal-header'>
      ...
      <Modal.Body>
        <Row>
          ...
          <Col md={6}>
            ...
            <div className="s-Opp-Notesection"></div>
            <hr />
            <p><strong>Comments with Date: </strong>{lead?.comments}</p>
            <div className="s-Opp-Commentsection"></div>
          </Col>
        </Row>
      </Modal.Body>
      <Modal.Footer className='s-LeadOppView-footer'>
        <Button className='s-LeadOppView-btn-secondary' onClick={handleClose}>
          Close
        </Button>
        <Button className='s-LeadOppView-btn-primary' >Edit</Button>
      </Modal.Footer>
    </Modal>
  );
};
