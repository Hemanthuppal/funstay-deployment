...
const MangerLeadView = ({ show, handleClose, lead, handleEdit }) => {
  const currentDate = new Date().toISOString().split("T")[0]; // Get current date in YYYY-MM-DD format

    return (
      <Modal show={show} onHide={handleClose} size="lg">
        <Modal.Header closeButton className='m-LeadView-modal-header'>
        ...
        <Modal.Body>
          <Row>
            ...
            <Col md={6}>
              ...
              <div className="m-Notesection"></div>
              <hr />
              <p><strong>Comments with Date: </strong>{lead?.comments}</p>
              <div className="m-Commentsection"></div>
            </Col>
          </Row>
        </Modal.Body>
        <Modal.Footer className='m-LeadView-footer'>
          <Button className='m-LeadView-btn-secondary' onClick={handleClose}>
            Close
          </Button>
          <Button className='m-LeadView-btn-primary' >Edit</Button>
        </Modal.Footer>
      </Modal>
    );
};

export default MangerLeadView;
